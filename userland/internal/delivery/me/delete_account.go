...
func (d *DeliveryMe) DeleteAccount(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	var body model.DeleteAccountRequest
	...
	res, err := d.me.DeleteAccount(ctx, userId.(string), body)
	if err != nil {
		e, ok := err.(*myerror.Error)
		if !ok {
			response.Write(w, http.StatusInternalServerError, "Our server encounter a problem.", nil, "BAD-ERROR")
